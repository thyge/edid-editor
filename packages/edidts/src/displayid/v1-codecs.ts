// packages/edidts/src/displayid/v1-codecs.ts
//
// DisplayID 1.x per-block codecs, split from v1-blocks.ts so the shared
// `DISPLAYID_BLOCK_CODECS` registry (in blocks.ts) can reference the v1
// decode/encode functions without an import cycle: blocks.ts imports this
// leaf module; v1-blocks.ts (the walker + dispatch delegates) imports the
// registry from blocks.ts. Neither direction creates a cycle because this
// file imports only types.
//
// Field layouts cross-checked against edid-decode parse-displayid-block.cpp
// (the `version < 0x20` arm of each parser).
...
export function isTypedV1ProductIdentificationBlock(
  block: DisplayIdDataBlock,
...
export function isTypedV1DisplayParametersBlock(
  block: DisplayIdDataBlock,
...
export function isTypedV1TypeITimingBlock(
  block: DisplayIdDataBlock,
...
export function isTypedV1TiledDisplayTopologyBlock(
  block: DisplayIdDataBlock,

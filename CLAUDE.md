...
- **`eedid/index.ts`** — Public surface for the EEDID module: re-exports `EEDID`, the `Extension` union, type guards, `getCEAExtension`, `getDisplayIdExtension`, `decodeCtaExtensionBlock`, `collectTimingsByPriority` / `TIMING_PRIORITY_RANK` (VESA E-EDID A2 §5 timing-priority policy), `collectVideoModeRefs`, and the `TimingEntry`/`TimingSource` types.
...
- **`cta/`** — CTA-861 codec. `extension-block.ts` (`ExtensionBlockParser`) owns the 0x02/0x10/0xF0 dispatch and the short data-block (tags 0x01–0x05/0x07) decode/encode; CEA detailed timings route through the shared full `DetailedTiming` codec. `video-timing-block.ts` is CTA-internal (the VTB arm). `cta-extended-blocks.ts` holds the tag-keyed `EXTENDED_BLOCK_CODECS` registry (with an `OPAQUE_EXTENDED_BLOCK` default) for extended-tag data blocks. Vendor-specific blocks are decoded via OUI-keyed registries: `vsdb/` for tag 0x03 (HDMI 1.4, HDMI Forum, Microsoft HMD, AMD, MHL) and `vsvdb/` for tag 0x07 ext 0x01 (Dolby Vision, HDR10+).
- **`displayid/`** — DisplayID section parser (v2.0 and v1.x). `section.ts` decodes/encodes a `DisplayIdSection` and chain-walks multi-section extensions (`decodeDisplayIdSections`); it dispatches on the version byte (v2.0 vs v1.x `0x10`–`0x1F`). `blocks.ts` holds the tag-keyed `DISPLAYID_BLOCK_CODECS` registry (v2 + v1 merged; tag spaces don't overlap) with an `OPAQUE_DISPLAYID_BLOCK` default; `product-identification.ts`, `display-parameters.ts`, the `type-*-timing.ts` / `adaptive-sync.ts` / `ar-vr.ts` / `brightness-luminance.ts` / `tiled-topology.ts` / `stereo-interface.ts` / `interface-features.ts` / `container-id.ts` / `cta-displayid.ts` files define the per-tag block types; `vendor-specific.ts` holds an OUI-keyed vendor registry (`DISPLAY_ID_VENDOR_DECODERS`/`ENCODERS`, with VESA DisplayPort as a registered codec); `v1-blocks.ts`/`v1-codecs.ts` hold the v1.x block walker and per-tag v1 codecs.
...
State flow: UI mutates fields on the `useEDID` composable's `edid` instance (a `shallowRef<EEDID | null>` wrapped in `reactive()` on load, so `base: EDID` and `extensions: Extension[]` are deeply reactive). `edidData` is a `computed<Uint8Array | null>` — `EEDID.encode(edid.value)` is its only expression, and because the EEDID tree is a `reactive()` proxy, every property read inside `encode` registers as the computed's dependency, so any deep mutation lazily re-encodes on the next render. This is the only encode site. `App.vue` (and all view components) never call `EEDID.encode()` directly (the former `syncEdid()` glue, then a module-level deep watcher + `suppressEncode` flag, were both retired in favor of the computed). The encode chain is pure (writes only to a fresh `Uint8Array`, never mutates the instance), so the computed cannot loop. Loading a file sets `edid.value = reactive(EEDID.decode(bytes))`; the hex view therefore shows canonical (re-encoded) bytes immediately on load rather than the uploaded file's raw bytes — for byte-identical round-trippers (the common case) there is no visible difference.

### UI Structure

- **`App.vue`** — Root layout with `TopNav`, `LeftNav`, and `HexViewer`. A hardcoded EDID hex string is parsed on mount for demo/development purposes. Mutations bind inline to the reactive `EEDID` tree; the `useEDID` `edidData` computed re-encodes and `HexViewer` renders it — there is no manual `syncEdid()` call.
- **`src/composables/useEDID.ts`** — Single composable that owns the EEDID instance (`edid: shallowRef<EEDID | null>`, wrapped in `reactive()` on load so nested fields are deeply tracked) and the derived bytes (`edidData: computed<Uint8Array | null>` = `EEDID.encode(edid.value)`). The encode is lazy/cached and re-runs on any deep mutation; there is no watcher and no `suppressEncode` guard. Also owns the loaders (`loadFromHex`, `loadFromFile`, `loadFromBytes`, `createBlankEdid`, `clear`). There is no Pinia store; the composable is the only state holder.
...
- **Reactivity caveat:** Because the core data uses plain classes and `Uint8Array`, Vue cannot detect deep mutations on raw bytes. `useEDID` handles this by wrapping the EEDID tree in `reactive()` and making `edidData` a `computed` over `EEDID.encode(edid.value)` — the proxy makes every field read inside `encode` a tracked dependency, so deep mutations mark the computed dirty and it lazily re-encodes on the next render. The computed is read-only (`edidData.value = ...` is a type error), which prevents manual encode call sites from reappearing.
...
- **No browser automation for verification:** Do not drive the app with Playwright/Selenium or capture screenshots for verification — the assistant cannot interpret images. Verify UI work with `npm run build` (type-check + build), grep/static review of the changed code, and by asking the user to confirm runtime behavior in their browser.
- **No Playwright/screenshot verification:** Do not use Playwright or headless-browser screenshots to verify UI changes. Verify with `npm run build` (vue-tsc type-check) and manual inspection in the dev server.

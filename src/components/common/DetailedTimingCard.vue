 ...
 * clock resolution (TASK-121); the geometry itself is never touched.
 */
 ...
 *  target refresh rate too high for the active resolution (TASK-108 AC #5).
 ...
 *  target that derives only the pixel clock from it (TASK-121). */
const customMode = computed(() => state.mode === 'custom')
/** "Target refresh rate" authoring mode (TASK-108): the refresh input is the
...
function onModeChange(event: Event): void {
  ...
  if (mode === 'custom') {
    ...
    // from the present timing (TASK-121 AC #4); Custom owns the geometry, so
...
/**
 * Custom-mode target-rate apply (TASK-121): derive ONLY the pixel clock from
...
function onRefreshChange(v: string | number): void {
  ...
  // the input cannot even ask for an unachievable rate (TASK-108 AC #5); the
  ...
  <div class="rounded-2xl border border-border/60 bg-card shadow-sm scroll-mt-6">
    ...
    >
      <!-- Top controls row: Pixel Clock | Refresh Rate | Margins. Always
           ...
           geometry (TASK-121); in CEA-861 mode it stays a read-only mirror of
